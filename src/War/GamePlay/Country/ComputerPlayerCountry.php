...
class ComputerPlayerCountry extends BaseCountry {
  ...
  public function chooseToAttack(): ?CountryInterface {
    ...
    $neighborUntrated = $this -> getNeighbors();
    $listNeighbor = [];
    foreach( $neighborUntrated as $index){
      if ($index -> isConquered() === false){
        $listNeighbor[] = $index;
      }
    }
    $listNeighbor[] = NULL;
    $sizeNeighbor = count($listNeighbor);
    $sortedNumber = rand(0, $sizeNeighbor -1);
    return $listNeighbor[$sortedNumber];
  }
